...
export default function App() {
  const [screen, setScreen] = useState("quote"); // quote | history | update
  ...
  const Tab = ({ id, label }) => (
    <button onClick={() => setActiveTab(id)} style={{ flex: 1, padding: "7px 8px", fontSize: 12, cursor: "pointer", borderRadius: "var(--radius)", background: activeTab === id ? "var(--text-primary)" : "transparent", color: activeTab === id ? "var(--bg)" : "var(--text-muted)", border: "none", fontWeight: activeTab === id ? 500 : 400 }}>
      {label}
    </button>
  );

  const PriceBlock = ({ label, opening, fallback, floor, openingMargin, fallbackMargin, floorMargin, cost, time, includes, highlight }) => (
      ...
      <div style={{ background: "var(--surface-2)", border: "0.5px solid var(--border)", borderRadius: 12, padding: "1.25rem", display: "flex", flexDirection: "column", gap: 16 }}>
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
            ...
            <button onClick={() => address && window.open(`https://www.google.com/maps/search/${encodeURIComponent(address)}`, "_blank")} style={{ padding: "0 14px", background: "var(--surface-1)", color: "var(--text-primary)", border: "0.5px solid var(--border)" }}>
        ...
        <div>
          ...
          <textarea value={extraNotes} onChange={e => setExtraNotes(e.target.value)} placeholder="Pool, colonial windows, narrow gate, lots of trees..." rows={2} style={{ width: "100%", marginTop: 6, fontSize: 14, resize: "vertical", boxSizing: "border-box" }} />
        </div>
        <button onClick={generateQuote} disabled={!address.trim() || loading} style={{ width: "100%", padding: "11px 0", fontSize: 14, fontWeight: 500, opacity: address.trim() && !loading ? 1 : 0.5, cursor: address.trim() && !loading ? "pointer" : "not-allowed" }}>
          {loading ? <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><i className="ti ti-loader-2" style={{ fontSize: 16, animation: "spin 1s linear infinite" }} />Researching property...</span> : <span><i className="ti ti-search" style={{ fontSize: 15, marginRight: 6 }} />Generate quote ↗</span>}
        </button>
        ...
        <div ref={resultRef} style={{ marginTop: 20 }}>
          ...
          <div style={{ background: "var(--surface-2)", border: "0.5px solid var(--border)", borderRadius: 12, padding: "1.25rem", marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              ...
              <button onClick={saveQuoteAsJob} style={{ fontSize: 12, padding: "6px 12px", background: "var(--surface-1)", color: "var(--text-primary)", border: "0.5px solid var(--border)", marginLeft: 8 }}>
                {saveMsg || "Save job"}
              </button>
            </div>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 12, background: "var(--surface-1)", padding: 4, borderRadius: "var(--radius)", border: "0.5px solid var(--border)" }}>
            ...
            <Tab id="gutters" label="🍂 Gutters" />
            <Tab id="combo" label="📦 Combo −10%" />
          </div>
        ...
        <div style={{ background: "var(--surface-2)", border: "0.5px solid var(--border)", borderRadius: 12, padding: "1.25rem", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <input type="number" value={selectedJob.quotedPrice || ""} onChange={e => set("quotedPrice", e.target.value)} placeholder="e.g. 390" style={{ width: "100%", marginTop: 6, boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={labelStyle}>Final price charged ($)</label>
              <input type="number" value={selectedJob.finalPrice || ""} onChange={e => set("finalPrice", e.target.value)} placeholder="e.g. 365" style={{ width: "100%", marginTop: 6, boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={labelStyle}>Estimated hours</label>
              <input type="text" value={selectedJob.estimatedHours || ""} onChange={e => set("estimatedHours", e.target.value)} placeholder="e.g. 3.5–4" style={{ width: "100%", marginTop: 6, boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={labelStyle}>Actual hours taken</label>
              <input type="number" step="0.5" value={selectedJob.actualHours || ""} onChange={e => set("actualHours", e.target.value)} placeholder="e.g. 3.5" style={{ width: "100%", marginTop: 6, boxSizing: "border-box" }} />
            </div>
          </div>

          <div>
            <label style={labelStyle}>Customer name</label>
            <input type="text" value={selectedJob.customerName || ""} onChange={e => set("customerName", e.target.value)} placeholder="e.g. Sarah Johnson" style={{ width: "100%", marginTop: 6, boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={labelStyle}>Customer phone</label>
            <input type="tel" value={selectedJob.customerPhone || ""} onChange={e => set("customerPhone", e.target.value)} placeholder="e.g. 0412 345 678" style={{ width: "100%", marginTop: 6, boxSizing: "border-box" }} />
          </div>
          <div>
            <label style={labelStyle}>Job notes</label>
            <textarea value={selectedJob.jobNotes || ""} onChange={e => set("jobNotes", e.target.value)} placeholder="e.g. Had colonial windows at front, took longer than expected. Customer wants quarterly service." rows={3} style={{ width: "100%", marginTop: 6, resize: "vertical", boxSizing: "border-box", fontSize: 14 }} />
          </div>

          {selectedJob.actualHours && selectedJob.finalPrice && (
          ...
          <button onClick={saveUpdate} style={{ width: "100%", padding: "11px 0", fontSize: 14, fontWeight: 500 }}>Save update</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: "20px 16px", fontFamily: "var(--font-sans)", background: "var(--surface-1)", minHeight: "100vh" }}>
      <style>{`
        ...
        input, textarea { padding: 10px 12px; border: 0.5px solid var(--border); border-radius: var(--radius); background: var(--bg); color: var(--text-primary); outline: none; font-family: var(--font-sans); font-size: 14px; width: 100%; }
      ...
      {/* Bottom nav */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--bg)", borderTop: "0.5px solid var(--border)", display: "flex", maxWidth: 600, margin: "0 auto" }}>
        {[
